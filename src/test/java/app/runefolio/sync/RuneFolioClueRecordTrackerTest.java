...
public class RuneFolioClueRecordTrackerTest
{
    ...
    @Test public void unmatchedRewardsNeverBecomeAnotherTiersRewards()
    {
        RuneFolioClueRecordTracker tracker = new RuneFolioClueRecordTracker();
        tracker.message("You have completed 4 hard Treasure Trails.", 10);
        Assert.assertNull(tracker.rewards(new JsonArray(), 11, "easy"));
        JsonObject result = tracker.poll(16);
        Assert.assertEquals("hard", result.get("tier").getAsString());
        Assert.assertFalse(result.has("items"));
        Assert.assertNull(tracker.poll(17));
    }
}
